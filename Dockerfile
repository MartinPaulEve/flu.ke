...
# track samples). nano: a convenience editor for poking around inside the
# container. Installed here, early, so the layer is cached.
RUN apt-get update \
 && apt-get install -y --no-install-recommends ffmpeg nano \
